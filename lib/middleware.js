 ...
 */
exports.publishData = function(db) {
  return function(req, res, next) {
    var options = {
 ...
 */
exports.publishMake = function(make, hostName) {
  return function(req, res, next) {
    var options = {
      withAuthor: req.session.email,
      withTags: ["thimble"],
      // TODO: it seems hostName causes a parsing error when passed in.
      // Something to do with the colon. Escaping for now.
      withUrl: require('querystring').escape( hostName + "/remix/" + req.body['original-url'] + req.publishId )
    };
    // connect to makeAPI and publish
    make.publish(options, function() {
      next();
    });
  }
};
